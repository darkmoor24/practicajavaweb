...
public interface IProveedorProductoRepository extends JpaRepository<ProveedorProducto, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "{ CALL spAgregarNuevoProveedorProducto(:claveProveedor, :costoProveedor, :idProveedor, :idProducto) }", nativeQuery = true)
    void addProveedorProducto(String claveProveedor, Double costoProveedor, int idProveedor, int idProducto);

    @Query(value = "{ CALL spObtenerProveedoresProducto(:idProducto) }", nativeQuery = true)
    ArrayList<ProveedorProducto> getProveedoresProducto(int idProducto);

    @Transactional
    @Modifying
    @Query(value = "{ CALL spActualizarProveedorProducto(:claveProveedor, :costoProveedor, :idProveedor, :idProveedorProducto) }", nativeQuery = true)
